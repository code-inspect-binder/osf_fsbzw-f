...
add.time <- function(dur){

	currTime <<- currTime + dur

	if(getActivation){
	  a.out = c(round(currSim), currStep, round(currTime*1000),
...
# this is to ensure that match remains '0' even when there is no '=retrieval' (i.e., when length(names(lhs)==0 in the next while loop)
		...
		if(match == 1){
			if(ret.que[i,2] >= par.rt){
				add.time(ret.latency(ret.que[i,2]))
...
#	dt.latency <<- as.data.frame(output.latency)
	...
	colnames(dt.out) <<- c("simNo", "stepNo", "time", "knopf.act", "flasche.act", "ballon.act", "blume.act")
